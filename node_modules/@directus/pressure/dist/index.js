...
var PressureMonitor = class {
	...
	get overloaded() {
		if (this.options.maxMemoryHeapUsed && this.memoryHeapUsed > this.options.maxMemoryHeapUsed) return true;
		if (this.options.maxMemoryRss && this.memoryRss > this.options.maxMemoryRss) return true;
		if (this.options.maxEventLoopDelay && this.eventLoopDelay > this.options.maxEventLoopDelay) return true;
		if (this.options.maxEventLoopUtilization && this.eventLoopUtilization > this.options.maxEventLoopUtilization) return true;
		return false;
	}
	updateUsage() {
...
const handlePressure = (options) => {
	const monitor = new PressureMonitor(options);
	return (_req, res, next) => {
		if (monitor.overloaded) {
			if (options.retryAfter) res.header("Retry-After", options.retryAfter);
			return next(options?.error ?? /* @__PURE__ */ new Error("Pressure limit exceeded"));
		}
		return next();
	};
};
...
//#endregion
export { PressureMonitor, handlePressure };