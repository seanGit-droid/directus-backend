...
//#region src/errors/failed-validation.ts
const messageConstructor = (extensions) => {
	...
	if ("valid" in extensions) switch (extensions.type) {
		case "eq":
	...
	if ("invalid" in extensions) switch (extensions.type) {
		case "neq":
	...
	if ("substring" in extensions) switch (extensions.type) {
		case "contains":
		case "icontains":
...
const FailedValidationError = createError("FAILED_VALIDATION", messageConstructor, 400);

//#endregion
//#region src/utils/joi-to-error-extensions.ts
const joiValidationErrorItemToErrorExtensions = (validationErrorItem, path) => {
	const extensions = {
		field: validationErrorItem.path[0],
		path: [...path ?? [], ...validationErrorItem.path.slice(1)]
	};
	const joiType = validationErrorItem.type;
	if (joiType.endsWith("only")) if (validationErrorItem.context?.["valids"].length > 1) {
		extensions.type = "in";
	...
	if (joiType.endsWith("invalid")) if (validationErrorItem.context?.["invalids"].length > 1) {
		extensions.type = "nin";
	...
	if (joiType.endsWith("required") || joiType.endsWith(".base")) extensions.type = "required";
	if (joiType.endsWith(".pattern.base")) {
		extensions.type = "regex";
		extensions.invalid = validationErrorItem.context?.value;
	}
	if (joiType === "number.unsafe") extensions.type = "unsafe";
	...
	if (!extensions.type) throw new Error(`Couldn't extract validation error type from Joi validation error item`);
	return extensions;
};
