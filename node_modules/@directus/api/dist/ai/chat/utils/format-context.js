...
function groupAttachments(attachments) {
	...
	for (const att of attachments) switch (att.type) {
		case "visual-element":
...
function formatVisualElement(att) {
	...
	return `### ${collection}/${item} — "${escapeAngleBrackets(att.display)}"
...
function formatPrompt(att) {
	...
	if (snapshot.text) lines.push(escapeAngleBrackets(snapshot.text));
...
${promptBlocks}
	...
	if (context.page) {
		...
		if (page.item !== void 0) pageLines.push(`Item: ${escapeAngleBrackets(String(page.item))}`);
...
${elementLines}
	...
	if (attachments.length > 0) parts.push(`## Attachment Rules
