...
async function up(knex) {
	...
	for (const preset of presets) {
		if (preset.filters) {
			...
			if (newFilter._and.length > 0) await knex("directus_presets").update({ filter: JSON.stringify(newFilter) }).where("id", "=", preset.id);
		}
...
async function down(knex) {
	...
	for (const preset of presets) {
		if (preset.filter) {
			...
			if (oldFilters.length > 0) await knex("directus_presets").update({ filters: JSON.stringify(oldFilters) }).where("id", "=", preset.id);
		}
		if (preset.layout_query) {
			const layoutQuery = typeof preset.layout_query === "string" ? parseJSON(preset.layout_query) : preset.layout_query;
			for (const [layout, query] of Object.entries(layoutQuery)) {
				if (query.sort && Array.isArray(query.sort)) query.sort = query.sort?.[0] ?? null;
				layoutQuery[layout] = query;
			}
			await knex("directus_presets").update({ layout_query: JSON.stringify(layoutQuery) }).where("id", "=", preset.id);
		}
