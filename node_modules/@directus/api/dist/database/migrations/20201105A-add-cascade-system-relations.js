...
async function up(knex) {
	for (const update of updates) await knex.schema.alterTable(update.table, (table) => {
...
async function down(knex) {
	for (const update of updates) await knex.schema.alterTable(update.table, (table) => {
		for (const constraint of update.constraints) table.dropForeign([constraint.column]);
	});
}
