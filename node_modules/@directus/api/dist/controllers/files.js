...
const multipartHandler = (req, res, next) => {
	...
	*/
	...
	busboy.on("file", async (_fieldname, fileStream, { filename, mimeType }) => {
		...
		if (isMimeTypeAllowed(mimeType, env["FILES_MIME_TYPE_ALLOW_LIST"]) === false) return busboy.emit("error", new InvalidPayloadError({ reason: `File is of invalid content type` }));
		fileCount++;
