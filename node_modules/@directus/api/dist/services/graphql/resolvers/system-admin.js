	...
	schemaComposer.Mutation.addFields({
		create_fields_item: {
			...
			resolve: async (_, args) => {
				...
				await service.createField(args["collection"], args["data"], void 0, { attemptConcurrentIndex: Boolean(args["concurrentIndexCreation"]) });
		...
		update_fields_item: {
			...
			resolve: async (_, args) => {
				...
				if (isSystemField(args["collection"], args["field"])) {
					if (!systemFieldUpdateSchema.safeParse(args["data"]).success) throw new InvalidPayloadError({ reason: "Only \"schema.is_indexed\" may be modified for system fields" });
				}
		...
		update_fields_items: {
			...
			resolve: async (_, args) => {
				...
				for (const fieldData of args["data"]) if (isSystemField(args["collection"], fieldData["field"])) {
					if (!systemFieldUpdateSchema.safeParse(fieldData).success) throw new InvalidPayloadError({ reason: "Only \"schema.is_indexed\" may be modified for system fields" });
				}
