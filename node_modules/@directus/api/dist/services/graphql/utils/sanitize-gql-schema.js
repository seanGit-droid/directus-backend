...
function sanitizeGraphqlSchema(schema) {
	...
	return {
		...
		relations: schema.relations.filter((relation) => {
			if (relation.collection && !collectionExists(relation.collection)) return skipRelation(relation);
			if (relation.related_collection && !collectionExists(relation.related_collection)) return skipRelation(relation);
			if (relation.meta) {
				if (relation.meta.many_collection && !collectionExists(relation.meta.many_collection)) return skipRelation(relation);
				if (relation.meta.one_collection && !collectionExists(relation.meta.one_collection)) return skipRelation(relation);
				if (relation.meta.one_allowed_collections && relation.meta.one_allowed_collections.some((allowed_collection) => !collectionExists(allowed_collection))) return skipRelation(relation);
			}
			return true;
		})
	};
