...
var SharesService = class extends ItemsService {
	...
	async updateMany(keys, data, opts) {
		...
		if (("item" in data || "collection" in data) && this.accountability) {
			let shares = [data];
			if (!("item" in data) || !("collection" in data)) shares = await this.knex.select("collection", "item").from("directus_shares").whereIn("id", keys);
			...
			for (const share of shares) {
				...
				itemsByCollection.set(collection, (itemsByCollection.get(collection) ?? /* @__PURE__ */ new Set()).add(item));
			}
			for (const [collection, items] of itemsByCollection) await validateAccess({
				accountability: this.accountability,
				action: "share",
				collection,
				primaryKeys: [...items]
