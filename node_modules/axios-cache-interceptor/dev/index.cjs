...
var httpVary = require('http-vary');
...
const defaultHeaderInterpreter = (headers, location) => {
  ...
  if (cacheControl) {
    const cc = cacheParser.parse(String(cacheControl));
    if (
...
function defaultRequestInterceptor(axios) {
  const onFulfilled = async config => {
    ...
    cache.data.headers[Header.Vary]) {
      ...
      const vary = Array.isArray(config.cache.vary) ? config.cache.vary : httpVary.parse(cache.data.headers[Header.Vary]);
      // Compares current request headers with cached vary headers (meta.vary)
      if (vary && vary !== '*' && !httpVary.compare(vary, (_cache$data$meta = cache.data.meta) == null ? void 0 : _cache$data$meta.vary, config.headers)) {
    ...
    if (cache.state === 'loading') {
      ...
      try {
        ...
        if (config.cache.vary !== false && (_state$data$meta = state.data.meta) != null && _state$data$meta.vary && state.data.headers[Header.Vary]) {
          const vary = Array.isArray(config.cache.vary) ? config.cache.vary : httpVary.parse(state.data.headers[Header.Vary]);
          // Compare vary headers - if mismatch, make own request
          if (vary && vary !== '*' && !httpVary.compare(vary, state.data.meta.vary, config.headers)) {
            if (true) {
              axios.debug({
                id: config.id,
...
function defaultResponseInterceptor(axios) {
  ...
  const onFulfilled = async response => {
    ...
    if (cacheConfig.vary !== false && response.headers[Header.Vary]) {
      const vary = Array.isArray(cacheConfig.vary) ? cacheConfig.vary : httpVary.parse(response.headers[Header.Vary]);
    ...
    }
    {
      axios.debug({
        ...
        data: {
          ttl,
          staleTtl,
          interpretHeader: cacheConfig.interpretHeader
        }
      });
    }
    const newCache = {
...
function buildWebStorage(storage, prefix = 'axios-cache-', maxStaleAge = 60 * 60 * 1000) {
  ...
  return buildStorage({
    ...
    set: (key, value) => {
      const result = _try.Result.try(save, key, value);
      if (result.ok) {
        return;
      }
      ...
      const retry = _try.Result.try(save, key, value);
      if (retry.ok) {
        return;
      }
      // we cannot hide non quota errors
      ...
      for (const item of descItems) {
        ...
        const lastTry = _try.Result.try(save, key, value);
        if (lastTry.ok) {
          return;
        }
        // we cannot hide non quota errors
